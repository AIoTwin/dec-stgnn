...
def data_preparate(args, device):
    ...
    return cln_adj_matrix, train_iters, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore
...
def calculate_flops_for_epoch(model, neighbour_indices, cln_id, cln_models):
    # Total number of parameters in the model
    total_params = sum(p.numel() for p in model.parameters())

    cln_neighbour_models = [cln_models[i] for i in neighbour_indices]
    # print(f"cln_neighbour_models: {len(cln_neighbour_models)}")
    
    # Number of neighbours
    num_neighbours = len(cln_neighbour_models)
    
    # FLOPs for parameter addition (num of neighbours + 1 local/own model)
    flops_add = total_params * (num_neighbours + 1)
    
    # FLOPs for averaging
    flops_avg = total_params
    
    # Total FLOPs
    total_flops = flops_add + flops_avg
    
    print(f"FLOPs for server-free aggregation cloudlet {cln_id}: {total_flops:,}")
    return total_flops
...
if __name__ == "__main__":
    ...
    cln_adj_matrix, train_iters, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore = data_preparate(args, device)
    ...
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, val_iters, test_iters):
        ...
        neighbour_cln_indices = []
        if len(cln_adj_matrix) != 0:
            cln_neighbour_indices = torch.nonzero(cln_adj_matrix[cln_id,:] == 1).squeeze().tolist()
            if (isinstance(cln_neighbour_indices, int)):
                cln_neighbour_indices = [cln_neighbour_indices]
            neighbour_cln_indices = cln_neighbour_indices
        neighbour_cln_indices_list.append(neighbour_cln_indices)

        cln_id +=1

    print(f"neighbour_cln_indices_list: {neighbour_cln_indices_list}")

    cln_id = 0
    for cln_model, neighbour_cln_ids in zip(cloudlet_models, neighbour_cln_indices_list):
        calculate_flops_for_epoch(cln_model, neighbour_cln_ids, cln_id, cloudlet_models)
        cln_id +=1