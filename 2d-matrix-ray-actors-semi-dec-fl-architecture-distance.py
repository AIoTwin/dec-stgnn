...
from actors import cloudlet_ray, master_server
...
def cloudlet_train(args, master_actor, cln_actors, zscore):
    best_WMAPE = 100
    best_epoch = 0
    for epoch in range (args.epochs):
        for i in range(args.cloudlet_num):
            cln_actors[i].train.remote(epoch, master_actor)

        # Log validation metrics
        cln_done = []
        for cln_actor in cln_actors:
            cln_done.append(cln_actor.val_2d_data_metric.remote(epoch, zscore))

    for epoch in range(args.epochs):
        for cln_actor in cln_actors:
            cln_actor.train(epoch, master_actor)
            # Each cloudlet validate data using metrics and log it
            cln_actor.log_val_metrics(epoch, zscore)
        ...
        master_val_loss = master_actor.val()
        master_actor.log_val(master_val_loss, epoch)

        # Validate data using metrics and log it, along with variance on master server
        val_WMAPE = master_actor.log_val_metrics(epoch, zscore)
        master_actor.log_var_info(epoch)

        # Keep best model for testing for all cloudlets and master server using val_WMAPE from from master
        if (val_WMAPE < best_WMAPE):
            best_WMAPE = val_WMAPE
            best_epoch = epoch
            for cln_actor in cln_actors:
                cln_actor.store_new_best_model()
            
            master_actor.store_new_best_model()
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_ray-semi-dec-fl-distance")
    ...
    master_actor = master_server.MasterServer(model, cloudlet_models, loss, edge_index_master, test_iter_master, val_iter_master, writer, logs_folder)
    ...
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, val_iters, test_iters):
        ...
        cln_actor = cloudlet_ray.Cloudlet.remote(model, cln_model, cln_loss, cln_optimizer, cln_scheduler, cln_id, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter, [], logs_folder)
    ...
    best_epoch = cloudlet_train(args, master_actor, cln_actors, master_zscore)

    master_actor.test_2d_data_using_best_model(master_zscore, args, best_epoch)

    for cln_actor in cln_actors:
        cln_actor.test_2d_data_using_best_model.remote(master_zscore, args, best_epoch)
