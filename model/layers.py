...
# Adjust the number of input channels to match the desired number of output channels
class Align(nn.Module):
    def __init__(self, c_in, c_out):
        super(Align, self).__init__()
        self.c_in = c_in
        self.c_out = c_out
        # define a 2D convolutional layer with kernel size (1, 1)
        # weight and bias is here!!!
...
class CausalConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, enable_padding=False, dilation=1, groups=1, bias=True):
        kernel_size = nn.modules.utils._pair(kernel_size) # convert to a pair
        stride = nn.modules.utils._pair(stride) # convert to a pair
        dilation = nn.modules.utils._pair(dilation) # convert to a pair
        if enable_padding == True:
            # padding - dynamically adjust the padding of the input tenspr based on the dilation rate
            # it ensures that the convolution operation is casual
...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(self, Kt, c_in, c_out, act_func):
        ...
        self.relu = nn.ReLU() # init ReLU activation function
        self.silu = nn.SiLU() # init SiLU activation function
...
class ChebGraphConv(nn.Module):
    ...
    def reset_parameters(self):
        # reset trainable parameters using Kaiming uniform initalization for weights and uniform initalization for biases
...
class GraphConvLayerPyG(nn.Module):
    def __init__(self, graph_conv_type, c_in, c_out, Ks, bias):
        super(GraphConvLayerPyG, self).__init__()
        self.graph_conv_type = graph_conv_type
        self.c_in = c_in
        self.c_out = c_out
        self.align = Align(c_in, c_out)
        self.Ks = Ks
        if self.graph_conv_type == 'cheb_graph_conv':
            self.cheb_graph_conv = ChebConv(c_out, c_out, Ks, bias=bias)
        elif self.graph_conv_type == 'graph_conv':
            self.graph_conv = GCNConv(c_out, c_out, bias=bias)  
    ...
    def forward(self, data):
        ...
        x_gc_in = self.align(x)
        #bs, c_in, ts, n_vertex = x.shape
        x_gc_in = torch.permute(x_gc_in, (0, 2, 3, 1))
        if self.graph_conv_type == 'cheb_graph_conv':
            x_gc = self.cheb_graph_conv(x_gc_in, edge_index)
        elif self.graph_conv_type == 'graph_conv':
            x_gc = self.graph_conv(x_gc_in, edge_index)
        x_gc_in = torch.permute(x_gc_in, (0, 3, 1, 2))
        x_gc = x_gc.permute(0, 3, 1, 2)
        x_gc_out = torch.add(x_gc, x_gc_in)
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], act_func)
        ...
        self.tmp_conv2 = TemporalConvLayer(Kt, channels[1], channels[2], act_func)
        # instead of LayerNorm with n_vertex, use it just with channels[2], because if we leave it as is, this means we do LayerNorm on all data (because of n_vertex) -> (or maybe use BatchNorm??)
        self.tc2_ln = nn.LayerNorm([n_vertex, channels[2]]) 
        self.relu = nn.ReLU()
        # Regularization technique to prevent overfitting and improve generalization ability of the model
        # It randomly sets a fraction of input units to zero during training -> introduces noise to the network -> helps prevent the network from relying too much on specific features or relationship within the training data
...
class STConvBlockPyG(nn.Module):
    def __init__(self, Kt, Ks, last_block_channel, channels, act_func, graph_conv_type, bias, droprate):
        super(STConvBlockPyG, self).__init__()
        # 1st layer - weights and biases
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], act_func)
        # 2nd layer - graph convolution
        self.graph_conv = GraphConvLayerPyG(graph_conv_type, channels[0], channels[1], Ks, bias)
        # 3rd layer - temporal convolution
        self.tmp_conv2 = TemporalConvLayer(Kt, channels[1], channels[2], act_func)
    ...
    def forward(self, data):
        #print(f"data num of elements: {len(data)}")
        #x, edge_index = data # fetch x and edge_index
        x, edge_index = data.x, data.edge_index
        x = self.tmp_conv1(x) # update x
        #data = (x, edge_index) # update data because "x" isn't being updated
        data = models.Data(x, edge_index)
        x = self.graph_conv(data) # send updated data
        x = self.relu(x)
        x = self.tmp_conv2(x)
        x = self.tc2_ln(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x = self.dropout(x)
        data = models.Data(x, edge_index) # return updated data
...
class OutputBlock(nn.Module):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, act_func, bias, droprate):
        ...
        # weights and biases -> tmp_conv1, fc1, fc2
        self.tmp_conv1 = TemporalConvLayer(Ko, last_block_channel, channels[0], act_func)
    ...
    def forward(self, x):
        # Forward pass through the temporal conv layer
        x = self.tmp_conv1(x)
        # Layer normalization
        x = self.tc1_ln(x.permute(0, 2, 3, 1))
        # Forward pass through 1st fully connected layer
        x = self.fc1(x)
        # ReLU act func
        x = self.relu(x)
        # Dropout regularization
        x = self.dropout(x)
        # Forward pass through 2nd fully connected layer and paermute the dimensions
        x = self.fc2(x).permute(0, 3, 1, 2)

        return x

class OutputBlockPyG(nn.Module):
    def __init__(self, Ko, last_block_channel, channels, end_channel, act_func, bias, droprate):
        super(OutputBlockPyG, self).__init__()
        self.tmp_conv1 = TemporalConvLayer(Ko, last_block_channel, channels[0], act_func)
        self.fc1 = nn.Linear(in_features=channels[0], out_features=channels[1], bias=bias)
        self.fc2 = nn.Linear(in_features=channels[1], out_features=end_channel, bias=bias)
