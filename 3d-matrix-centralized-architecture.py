...
def data_preparate(args, device):
    ...
    train, val, test = utility.zscore_preprocess_3d_data(zscore, train, val, test, use_fit_transform=True)

    # transform data into input-output pairs suitable for training
    x_train, y_train = dataloader.data_3d_transform(train, args.n_his, args.n_pred, device)
    x_val, y_val = dataloader.data_3d_transform(val, args.n_his, args.n_pred, device)
    x_test, y_test = dataloader.data_3d_transform(test, args.n_his, args.n_pred, device)
    ...
    return edge_index, zscore, train_iter, val_iter, test_iter, blocks
...
def train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter, edge_index, writer, logs_folder):
    ...
    for epoch in range(args.epochs):
        ...
        total_num_of_parameters += num_of_parameters
        ...
        utility.save_val_logs(logs_folder, "central", epoch+1, optimizer.param_groups[0]['lr'], l_sum / n, val_loss, gpu_mem_alloc)
...
def test(zscore, loss, model, test_iter, args, edge_index, logs_folder):
    ...
    for node_feature_to_evaluate in args.node_features_to_evaluate:
        ...
        utility.save_test_logs(logs_folder, f"centralized_node-feature-{node_feature_to_evaluate}", test_MSE, test_MAE, test_RMSE, test_WMAPE)
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_{args.n_pred * 5}min_centralized")

    edge_index, zscore, train_iter, val_iter, test_iter, blocks = data_preparate(args, device)
    edge_index = edge_index.to(device)
    loss, es, model, optimizer, scheduler = prepare_model(args, blocks)
    train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter, edge_index, writer, logs_folder)
