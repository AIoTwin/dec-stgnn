...
def cloudlet_train(args, cln_actors, cln_models, esClnList, zscore):
    for epoch in range(args.epochs):
        ...
        # Check if averaged model is the best
        for cln_actor in cln_actors:
            ray.get(cln_actor.check_best_model_using_metrics.remote(epoch, zscore))

        # Log validation metrics
        for cln_actor in cln_actors:
            ray.get(cln_actor.val_2d_data_metric.remote(epoch, zscore))

        # Log transfer size of cloudlet neighbour model parameters
        for cln_actor in cln_actors:
            cln_actor.log_transfer_neighbour_model_parameters_gossip_learning.remote(epoch)
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_{args.n_pred * 5}min_ray-semi-dec-gossip-learning-distance")
    ...
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, val_iters, test_iters):
        cln_loss, cln_optimizer, cln_scheduler = setup_loss_optimizer_and_scheduler(args, cln_model)
        ...
        ray.get(cln_actor.init_loss_optimizer_scheduler.remote(args)) # Switch to SGD optimizer for gossip learning if Adam doesn't work!!!!!!!!!!!!
    ...
    cloudlet_train(args, cln_actors, cloudlet_models, esClnList, master_zscore)

    for cln_actor in cln_actors:
        cln_actor.test_2d_data_using_best_model.remote(master_zscore, args)

    for cln_actor in cln_actors:
        ray.get(cln_actor.flush_writer.remote())

    # Finish wandb for all cloudlets
    for cln_actor in cln_actors:
        ray.get(cln_actor.finish_wandb.remote())
