...
def data_preparate(args, device):
    ...
    for cln_nodes in cln_nodes_list:
        ...
        # cross_cloudlet_edges_map = cross_cloudlet_edge_mask.nonzero().squeeze()

        cross_cloudlet_edge_index_list.append(cross_cloudlet_edge_index)
        # cross_cloudlet_edges_map_list.append(cross_cloudlet_edges_map)
        ...
        print(f"cross_cloudlet_edge_mask size: {len(cross_cloudlet_edge_mask)}")
    ...
    return cln_adj_matrix, cln_train_list, len_train, train_iters, x_train, y_train, val_iters, test_iters, cln_edge_index_list, cross_cloudlet_edge_index_list, cross_cloudlet_edge_mask_list, cln_node_map_list, master_zscore
...
def cloudlet_train(args, cln_actors, cln_models, zscore, len_train):
    num_epochs = int((len_train - args.end_of_initial_data_index) / args.data_per_step)

    for epoch in range(1, num_epochs + 1):
        for i in range(args.cloudlet_num):
            cln_models[i] = cln_actors[i].online_train_no_master.remote(epoch, num_epochs)

        # Calcualte edge scores
        cln_done = []
        for cln_actor in cln_actors:
            if (epoch < num_epochs):
                ...
                # using full validation set, remove edges based on edge_scores and some other criteria, and calcualte validation metrics (WMAPE, MAE, RMSE, etc.)
                ray.get(cln_actor.exploit_edge_scores.remote(zscore))
    ...
    #     # Log transfer size of cloudlet neighbour model parameters
    #     for cln_actor in cln_actors:
    #         ray.get(cln_actor.log_transfer_neighbour_model_parameters.remote(cln_models, epoch))
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_ray-semi-dec-server-free-fl-distance_online-training_with-edge-score")

    cln_adj_matrix, cln_train_list, len_train, train_iters, x_trains, y_trains, val_iters, test_iters, cln_edge_index_list, cross_cloudlet_edge_index_list, cross_cloudlet_edge_mask_list, cln_node_map_list, master_zscore = data_preparate(args, device)
    ...
    cln_actors = []
    cln_id = 0
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_x_train, cln_y_train, cln_val_iter, cln_test_iter, cln_train_dataset, cross_cloudlet_edge_index, cross_cloudlet_edge_mask in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, x_trains, y_trains, val_iters, test_iters, cln_train_list, cross_cloudlet_edge_index_list, cross_cloudlet_edge_mask_list):
    ...
    cloudlet_train(args, cln_actors, cloudlet_models, master_zscore, len_train)
