...
def data_preparate(args, device):
    ...
    adj, _ = dataloader.load_adj_direct_0_1(args.dataset)
    ...
    return cln_train_list, len_train, train_iters, x_train, y_train, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore, train_iter_master, x_train_master, y_train_master, test_iter_master, val_iter_master, edge_index
...
def cloudlet_train(args, master_actor, cln_actors, zscore, len_train):
    num_epochs = int((len_train - args.end_of_initial_data_index) / args.data_per_step)

    for epoch in range(1, num_epochs + 1):
        for cln_actor in cln_actors:
            cln_actor.online_train(epoch, master_actor)
            # Each cloudlet validate data using metrics and log it
            cln_actor.log_validation_metric_for_online_training(epoch, zscore, num_epochs)
            # Log var info for each cloudlet
        ...
        for cln_actor in cln_actors:
            ...
            cln_num_of_parameters = sum(p.numel() for p in cln_actor.get_cln_model().parameters())
            total_num_of_parameters += cln_num_of_parameters
            total_size_in_bytes_per_epoch = (total_num_of_parameters) * 2
            # total_data_sent += total_size_in_bytes_per_epoch
            print(f"Epoch: {epoch:03d} | Size in bytes: {total_size_in_bytes_per_epoch} | in kilobytes: {total_size_in_bytes_per_epoch/1024} | in megabytes: {(total_size_in_bytes_per_epoch/1024)/1024}")
            utility.save_total_trainable_parameters_transfer_size(logs_folder, cln_actor.get_cln_id(), epoch, total_size_in_bytes_per_epoch)

        master_actor.average_model()
        master_actor.send_model_to_cloudlets(cln_actors)

        master_val_loss = master_actor.calculate_val_online(epoch)
        master_actor.log_val(master_val_loss, epoch)

        # Validate data using metrics and log it, along with variance on master server
        master_actor.log_validation_metric_for_online_training(epoch, zscore)
        master_actor.log_var_info(epoch-1)
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_semi-dec-fl-distance_online-training")

    cln_train_list, len_train, train_iters, x_trains, y_trains, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore, train_iter_master, x_train_master, y_train_master, test_iter_master, val_iter_master, edge_index_master = data_preparate(args, device)
    ...
    cln_actors = []
    cln_id = 0
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_x_train, cln_y_train, cln_val_iter, cln_test_iter, cln_train_dataset in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, x_trains, y_trains, val_iters, test_iters, cln_train_list):
        cln_loss, cln_optimizer, cln_scheduler = setup_loss_optimizer_and_scheduler(cln_model)
        cln_actor = cloudlet.Cloudlet(model, cln_model, cln_loss, cln_optimizer, cln_scheduler, cln_id, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter, logs_folder, cln_adj_matrix=[], train_dataset=cln_train_dataset, x_train=cln_x_train, y_train=cln_y_train, end_of_initial_data_index=args.end_of_initial_data_index, data_per_step=args.data_per_step, batch_size=args.batch_size)
        cln_actor.initialize_writer()
        cln_actors.append(cln_actor)
        cln_id +=1

    cloudlet_train(args, master_actor, cln_actors, master_zscore, len_train)
