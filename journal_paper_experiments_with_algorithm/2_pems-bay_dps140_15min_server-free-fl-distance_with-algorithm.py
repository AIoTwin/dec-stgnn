...
def get_parameters():
    ...
    parser.add_argument('--cloudlet_num', type=int, default=7, help='the number of cloudlets for semi-dec training (metr-la and pems-bay default: 10 | pemsd7-m default: 12)')
    ...
    parser.add_argument('--end_of_initial_data_index', type=int, default=29792, help='End of initial data for training dataset (model will train using datapoints 0-end_of_initial_data_index from all sensors) (METR-LA default: 16491) (PeMS-BAY default: 29792) (PeMSD7-M default: 4537)')
    parser.add_argument('--data_per_step', type=int, default=140, help='How much data will be taken from entire training dataset each "epoch" (each time model will be trained) (METR-LA default: 250) (PeMS-BAY default: 140) (PeMSD7-M default: 140)')
...
def data_preparate(args, device):
    ...
    # using dataset split rate as train: val: test = 80: 20: 00
    val_rate = 0.20
    test_rate = 0
    ...
    return adj.shape[0], cln_nodes_list, cross_cloudlet_nodes_list, cln_adj_matrix, cln_train_list, cln_val_list, len_train, train_iters, x_train, y_train, val_iters, test_iters, cln_edge_index_list, cross_cloudlet_edge_index_list, cross_cloudlet_edge_mask_list, cln_node_map_list, master_zscore
...
def cloudlet_train(args, cln_actors, cln_models, zscore, len_train, num_nodes, device):
    ...
    for epoch in range(0, num_epochs + 1):
        for cln_actor in cln_actors:
            # ray.get(cln_actor.choose_cross_cloudlet_nodes_by_distribution.remote(args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, zscore, 0.4))
            ray.get(cln_actor.choose_cross_cloudlet_nodes_by_distribution_with_adaptive_percentage.remote(args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, zscore))
        ...
        cln_done = []
        for cln_actor in cln_actors:
            masked_cross_cloudlet_nodes = ray.get(cln_actor.choose_random_cross_cloudlet_nodes_after_distribution.remote())
            masked_cln_edge_index = ray.get(cln_actor.remove_nodes_distribution.remote(masked_cross_cloudlet_nodes))
            masked_val_iter, masked_cln_edge_index, masked_cln_node_map = ray.get(cln_actor.create_val_iter_from_randomly_removed_cross_cloudlet_nodes_for_node_score_calculation.remote(masked_cln_edge_index, args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, num_epochs))
            cln_done.append(cln_actor.delta_error_between_original_and_masked_for_node_score_online_training.remote(epoch, zscore, num_epochs, masked_cln_edge_index, masked_cross_cloudlet_nodes, masked_val_iter, masked_cln_node_map))

        for cln_done_singal in cln_done:
            ray.get(cln_done_singal)

        for cln_actor in cln_actors:
            ray.get(cln_actor.log_var_info.remote(epoch))

    for i in range(args.cloudlet_num):
        ray.get(cln_actors[i].log_node_score_results.remote())
...
if __name__ == "__main__":
    ...
    root_dir = get_project_root()
    logs_folder = os.path.join(root_dir, 'logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_dps-{args.data_per_step}_server-free-fl-distance_online-training_with_algorithm")

    num_nodes, cln_nodes_list, cross_cloudlet_nodes_list, cln_adj_matrix, cln_train_list, cln_val_list, len_train, train_iters, x_trains, y_trains, val_iters, test_iters, cln_edge_index_list, cross_cloudlet_edge_index_list, cross_cloudlet_edge_mask_list, cln_node_map_list, master_zscore = data_preparate(args, device)
