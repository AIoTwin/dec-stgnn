...
def cloudlet_train(args, master_actor, cln_actors, zscore, len_train, num_nodes, device):
    ...
    for epoch in range(0, num_epochs + 1):
        for cln_actor in cln_actors:
            cln_actor.choose_cross_cloudlet_nodes_by_distribution_with_adaptive_percentage(args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, zscore)
...
if __name__ == "__main__":
    ...
    root_dir = get_project_root()
    logs_folder = os.path.join(root_dir, 'logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_dps-{args.data_per_step}_trad-fl-distance_online-training_with_algorithm")
