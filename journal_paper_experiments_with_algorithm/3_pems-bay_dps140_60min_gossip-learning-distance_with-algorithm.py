...
def cloudlet_train(args, cln_actors, cln_models, zscore, len_train, num_nodes, device):
    ...
    for epoch in range(1, num_epochs + 1):
        for cln_actor in cln_actors:
            ray.get(cln_actor.average_models_gossip_buffer.remote())
        ...
        # Send model to 1 random cloudlet buffer
        cln_done = []
        for cln_actor in cln_actors:
            cln_done.append(cln_actor.send_model_to_cloudlet_gossip.remote(cln_actors, cln_models))
...
if __name__ == "__main__":
    ...
    root_dir = get_project_root()
    logs_folder = os.path.join(root_dir, 'logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_dps-{args.data_per_step}_gossip-learning-distance_online-training_with_algorithm")
