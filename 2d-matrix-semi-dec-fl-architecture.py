...
def get_parameters():
    ...
    parser.add_argument('--epochs', type=int, default=5, help='epochs, default as 10000')
    ...
    parser.add_argument('--cloudlet_num', type=int, default=3, help='the number of cloudlets for semi-dec training, default as 10')
    ...
    # Calculate kernel size (Ko) based on:
...
# prepare dataset according to number of cloudlets (separate into args.cloudlet_num datasets)
# -> keep in mind about exchanging node embeddings during training between nodes that are in different cloudlets (maybe put nodes in 2 cloudlets at the same time?)
#   -> this means we'll need to have number of layers as well in order to know how many hops does node make in order to get node embedding info (and node feature)
# -> search for partitioning algorithm that will help me achieve this!!!!!!!!!!
# -> but 1st, I'll need to have a better look at the dataset -> and then create 10 separate datasets (or args.cloudet_num of datasets)
# return number of datasets equal to args.cloudlet_num
def data_preparate(args, device):
    ...
    # Everyone shares 1 zscore preprocessing
    ...
    # Test dataset for master (we don't need to train or validate master model, only test aggregated cloudlet models)
    ...
    return train_iters, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore, test_iter_master, edge_index
...
def cloudlet_train(cln_losses, args, cln_optimizers, cln_schedulers, es, master_model, cloudlet_models, train_iters, val_iters, cln_edge_index_list, cln_node_map_list, writer, logs_folder):
    total_data_sent = 0
    for epoch in range(args.epochs):
        ...
        for (cloudlet_model, train_iter, val_iter, cln_edge_index, cln_node_map, cln_loss, cln_optimizer, cln_scheduler) in zip(cloudlet_models, train_iters, val_iters, cln_edge_index_list, cln_node_map_list, cln_losses, cln_optimizers, cln_schedulers):
            ...
            for x, y in tqdm.tqdm(train_iter):
                y_pred = cloudlet_model(x, cln_edge_index).view(len(x), -1)  # [batch_size, num_nodes]
                y = y[...,cln_node_map]
            ...
            print('CLN: {:03d} | Epoch: {:03d} | Lr: {:.20f} |Train loss: {:.6f} | Val loss: {:.6f} | GPU occupy: {:.6f} MiB'.\
                format(cloudlet_id, epoch+1, cln_optimizer.param_groups[0]['lr'], l_sum / n, val_loss, gpu_mem_alloc))
...
def test(zscore, loss, model, test_iter, args, cln_edge_index, cln_node_map, cln_id, logs_folder):
    model.eval()
    test_MSE = utility.evaluate_pyg_model(model, loss, test_iter, cln_edge_index, cln_node_map)
    test_MAE, test_RMSE, test_WMAPE = utility.evaluate_pyg_metric(model, test_iter, zscore, cln_edge_index, cln_node_map)
    utility.save_test_logs(logs_folder, f"cln-{cln_id}", test_MSE, test_MAE, test_RMSE, test_WMAPE)
...
def test_master(zscore, loss, model, test_iter, args, edge_index, logs_folder):
    ...
    utility.save_test_logs(logs_folder, f"master-server", test_MSE, test_MAE, test_RMSE, test_WMAPE)
...
if __name__ == "__main__":
    ...
    train_iters, val_iters, test_iters, cln_edge_index_list, cln_node_map_list, master_zscore, test_iter_master, edge_index_master = data_preparate(args, device)
    ...
    # train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter, adj_matrix)
    cloudlet_train(cln_losses, args, cln_optimizers, cln_schedulers, es, model, cloudlet_models, train_iters, val_iters, cln_edge_index_list, cln_node_map_list, writer, logs_folder)    
    test_master(master_zscore, loss, model, test_iter_master, args, edge_index_master, logs_folder)
    # test each cloudlet
    i = 1
    for (cloudlet_model, test_iter, cln_edge_index, cln_node_map, cln_loss) in zip(cloudlet_models, test_iters, cln_edge_index_list, cln_node_map_list, cln_losses):
