...
def data_preparate(args, device):
    ...
    if ((len_train - args.end_of_initial_data_index) % args.data_per_step == 0):
        ...
        if (int((len_train - args.end_of_initial_data_index) / args.data_per_step) <= args.final_epoch_for_initial_edge_score_calc):
            print(f"args.final_epoch_for_initial_edge_score_calc has more epochs than model will train (will not be adaptive!!!)")
            print(f"args.final_epoch_for_initial_edge_score_calc: {args.final_epoch_for_initial_edge_score_calc}")
            exit(1)
    else:
...
def cloudlet_train(args, cln_actors, cln_models, zscore, len_train, num_nodes, device):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # At the beginning, calculate edge scores until certain epoch
        if (epoch <= args.final_epoch_for_initial_edge_score_calc):
        ...
        # After calculating edge scores, on the "final" epoch, remove edges based on edge scores
        if (epoch == args.final_epoch_for_initial_edge_score_calc):
            for cln_actor in cln_actors:
                # Remove edges based on edge scores and set it to be the new edge_index for the cloudlet
                # TODO - add criteria and check if criteria has been fulfilled
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_ray-semi-dec-server-free-fl-distance_online-training_with-adaptive-edge-score")
