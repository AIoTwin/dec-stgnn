...
if __name__ == "__main__":
    ...
    total_flops_clns = []
    for cln_model, cln_edge_index, cln_train_iter in zip(cloudlet_models, cln_edge_index_list, train_iters):
        cln_total_flops = calculate_flops_for_epoch(cln_model, cln_train_iter, cln_edge_index, device)
        total_flops_clns.append(cln_total_flops)

    i = 1
    for total_flops_cln in total_flops_clns:
        print(f"Cloudlet {i}: {total_flops_cln}")
        i = i + 1