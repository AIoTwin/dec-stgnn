...
dataset_choice = "pemsd7-m" # Change this depending on the dataset you want to plot (sensor and cloudlet locations)
datasets = {
    "metr-la": {"title": "METR-LA"},
    "pems-bay": {"title": "PeMS-BAY"},
    "pemsd7-m": {"title": "PeMS7-M"}
}
dataset = dataset_choice
title = datasets[dataset_choice]["title"]

experiment = "experiment_1"
plot_location = os.path.join(base_dir, f"logs/plots", f"locations_{dataset}_{experiment}_subplot.png")
...
if unassigned_sensors:
    ...
    ax.scatter(unassigned_sensor_data['longitude'], unassigned_sensor_data['latitude'], 
               c='black', marker='*', label='Unassigned Sensors')
...
os.makedirs(os.path.dirname(plot_location), exist_ok=True)
