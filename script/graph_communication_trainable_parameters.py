...
output_file_name = "plot_communication_model_transfer.png"
...
for i, relative_path in enumerate(csv_files):
    ...
    if os.path.isfile(path):
        ...
        csv_list = [os.path.join(path, file) for file in os.listdir(path) 
                    if file.endswith('.csv') and '_total_trainable_parameters_transfer_size' in file]
    else:
    ...
    if df_list:
        ...
        label = labels[i] if i < len(labels) else f'Model transfer size {i+1}'
...
plt.title('Model Transfer Size Over Epochs')
plt.legend()
