...
def data_preparate(args, device):
    ...
    for td in node_val_datas:
        node_val_iter = utils.data.DataLoader(dataset=td, batch_size=args.batch_size, shuffle=False)
    ...
    for td in node_test_datas:
        node_test_iter = utils.data.DataLoader(dataset=td, batch_size=args.batch_size, shuffle=False)
        node_test_iters.append(node_test_iter)

    return edge_index, edge_weight, zscore, len_train, train, x_train, y_train, val_iter, test_iter, node_x_trains, node_y_trains, node_val_iters, node_test_iters, nodes_edge_index_list, n_vertex
...
def train(loss, args, optimizer, scheduler, es, model, len_train, train_dataset, x_train, y_train, val_iter, edge_index, edge_weight, writer, logs_folder, zscore, node_x_trains, node_y_trains, node_val_iters, node_edge_indices, n_vertex):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch == num_epochs:
            ...
            # original_y_pred, modified_y_pred = utility.evaluate_partial_edge_influence(model, val_iter, zscore, edge_index, n_vertex)
            original_y_pred, modified_y_pred = utility.evaluate_partial_edge_influence_with_edge_weight(model, val_iter, zscore, edge_index, edge_weight, n_vertex)
            end_time = datetime.datetime.now()
            execution_time = str(end_time - start_time).split('.')[0]
            print(f"Execution time: {execution_time}")
            utility.save_y_pred_dict_to_csv(logs_folder, f"y_pred_original", original_y_pred)
            utility.save_modified_y_pred_to_csv_edge_weight(logs_folder, modified_y_pred)
...
if __name__ == "__main__":
    ...
    edge_index, edge_weight, zscore, len_train, train_dataset, x_train, y_train, val_iter, test_iter, node_x_trains, node_y_trains, node_val_iters, node_test_iters, nodes_edge_index_list, n_vertex = data_preparate(args, device)
    ...
    train(loss, args, optimizer, scheduler, es, model, len_train, train_dataset, x_train, y_train, val_iter, edge_index, edge_weight, writer, logs_folder, zscore, node_x_trains, node_y_trains, node_val_iters, nodes_edge_index_list, n_vertex)
