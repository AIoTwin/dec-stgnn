...
def get_parameters():
    ...
    parser.add_argument('--adj_matrix_type', type=str, default="no_neighbours", help="original | no_neighbours")
...
def data_preparate(args, device):
    ...
    for node_train, node_val, node_test in zip(node_train_list, node_val_list, node_test_list):
        _, node_val, _ = utility.zscore_preprocess_2d_data_1_node(zscore, node_train, node_val, node_test)
    ...
    for node_val in node_vals:
        x, y = dataloader.data_transform(node_val, args.n_his, args.n_pred, device)
        node_val_data = utils.data.TensorDataset(x, y)
        node_val_iter = utils.data.DataLoader(dataset=node_val_data, batch_size=args.batch_size, shuffle=False)
        node_val_iters.append(node_val_iter)

    return edge_index, zscore, train_iter, val_iter, test_iter, node_val_iters, nodes_edge_index_list, n_vertex
...
def train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter, edge_index, writer, logs_folder, zscore, best_model, n_vertex):
    best_WMAPE = 100
    best_epoch = 0
    for epoch in range(args.epochs):
        ...
        for node_idx, (val_mae_node, val_rmse_node, val_wmape_node) in enumerate(zip(val_mae_per_node, val_rmse_per_node, val_wmape_per_node)):
            utility.save_val_metric_logs(logs_folder, node_idx, epoch+1, val_mae_node, val_rmse_node, val_wmape_node)
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_centralized_individual-sensor_adj-{args.adj_matrix_type}")

    edge_index, zscore, train_iter, val_iter, test_iter, node_val_iters, nodes_edge_index_list, n_vertex = data_preparate(args, device)
    ...
    best_epoch = train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter, edge_index, writer, logs_folder, zscore, best_model, n_vertex)
