...
def cloudlet_train(args, cln_actors, cln_models, esClnList, zscore):
    for epoch in range(args.epochs):
        for i in range(args.cloudlet_num):
            cln_models[i] = cln_actors[i].train_no_master.remote(epoch, zscore)
        ...
        for cln_actor in cln_actors:
            cln_actor.log_transfer_neighbour_model_parameters.remote(cln_models, epoch)
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_{args.n_pred * 5}min_ray-semi-dec-server-free-fl")
