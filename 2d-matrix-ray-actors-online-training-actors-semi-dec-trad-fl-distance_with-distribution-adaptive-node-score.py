...
def get_parameters():
    ...
    parser.add_argument('--data_per_step', type=int, default=150, help='How much data will be taken from entire training dataset each "epoch" (each time model will be trained) (METR-LA default: 250) (PeMS-BAY default: 300) (PeMSD7-M default: 140)')
...
def cloudlet_train(args, master_actor, cln_actors, zscore, len_train, num_nodes, device):
    ...
    for epoch in range(1, num_epochs + 1):
        for cln_actor in cln_actors:
            cln_actor.choose_cross_cloudlet_nodes_by_distribution(args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, zscore)
        ...
        for cln_actor in cln_actors:
            masked_cross_cloudlet_nodes = cln_actor.choose_random_cross_cloudlet_nodes_after_distribution()
            masked_cln_edge_index = cln_actor.remove_nodes_distribution(masked_cross_cloudlet_nodes)
            masked_val_iter, masked_cln_edge_index, masked_cln_node_map = cln_actor.create_val_iter_from_randomly_removed_cross_cloudlet_nodes_for_node_score_calculation(masked_cln_edge_index, args.stblock_num, args.Ks, num_nodes, device, args.n_his, args.n_pred, args.batch_size, epoch, num_epochs)
            cln_actor.delta_error_between_original_and_masked_for_node_score_online_training(epoch, zscore, num_epochs, masked_cln_edge_index, masked_cross_cloudlet_nodes, masked_val_iter, masked_cln_node_map)

        for cln_actor in cln_actors:
            cln_actor.log_var_info(epoch)

    for cln_actor in cln_actors:
        cln_actor.log_node_score_results()
...
if __name__ == "__main__":
    ...
    logs_folder = os.path.join('logs', f"{current_datetime}_{args.dataset}_pred-{args.n_pred * 5}min_his-{args.n_his * 5}min_ray-semi-dec-trad-fl-distance_online-training_with-distribution-adaptive-node-score")
