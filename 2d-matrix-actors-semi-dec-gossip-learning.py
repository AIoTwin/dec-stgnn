...
def cloudlet_train(args, cln_actors, esClnList):
    for epoch in range(args.epochs):
        for cln_actor in cln_actors:
            cln_actor.average_models_gossip_buffer()

        for cln_actor in cln_actors:
            cln_actor.train_no_master(epoch)

        # After all cloudlets have trained for 1 epoch, send model to 1 random cloudlet buffer
        for cln_actor in cln_actors:
            cln_actor.send_model_to_cloudlet_gossip(cln_actors)

        # Log transfer size of cloudlet neighbour model parameters
        for cln_actor in cln_actors:
            cln_actor.log_transfer_neighbour_model_parameters_gossip_learning(epoch)
...
if __name__ == "__main__":
    ...
    for cln_model, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter in zip(cloudlet_models, cln_edge_index_list, cln_node_map_list, train_iters, val_iters, test_iters):
        ...
        cln_actor = cloudlet.Cloudlet(model, cln_model, cln_loss, cln_optimizer, cln_scheduler, cln_id, cln_edge_index, cln_node_map, cln_train_iter, cln_val_iter, cln_test_iter, logs_folder, cln_adj_matrix)
    ...
    cloudlet_train(args, cln_actors, esClnList)
